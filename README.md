...
- **Clear all renewal data** prompts Yes/No and clears all saved renewal lists, unsaved form values and removal/Undo state in this browser for this site. Other open tabs of the app synchronize saved lists and clear their forms when the list is cleared. It does not touch other apps or downloaded PDFs. Existing saved message snapshots are unchanged by formatting updates; re-add the policy to replace an older message.
- Saved lists use `localStorage` and survive closing/reopening the browser on the same device and site address. Existing tab-storage lists are migrated on first load when no local list exists; the old copy is removed only after successful saving. An empty local list prevents old tab data from returning after clearing. Unsaved form drafts remain temporary. Browser privacy settings, private browsing, or clearing site data can still remove saved lists; this is not cross-device storage. Storage failures are shown in the UI.
